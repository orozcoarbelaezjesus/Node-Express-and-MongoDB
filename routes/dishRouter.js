...
dishRouter.route("/:dishId/comments/:commentId")
    ...
    .post(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
        res.statusCode = 403;
        res.end("POST operation not suppo rted on /dishes/" + req.params.dishId + "/comments/" + req.params.commentId);
    })
        ...
        Dishes.findById(req.params.dishId)
            .then((dish) => {
                if (dish != null && dish.comments.id(req.params.commentId) != null) {
                    function position() {
                        let position;
                        for (let i = 0; i <= dish.comments.length - 1; i++) {
                    ...
                    let idAuthorComment = dish.comments[pos].author;
                    if (idUser.toString() == idAuthorComment.toString()) {
                        if (req.body.rating) {
